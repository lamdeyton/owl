...
function compileValueNode(value: any, node: Element, qweb: QWeb, ctx: CompilationContext) {
  if (value === "0") {
    if (ctx.parentNode) {
      // the c__0 magical variable is where we can find the result of the rendering
      // of  the body of the t-call.
      ctx.addLine(`c${ctx.parentNode}.push(...scope.c__0);`);
    }
    return;
  }
...
QWeb.addDirective({
  ...
  atNodeEncounter({ node, qweb, ctx }): boolean {
    ...
    if (ctx.parentNode) {
      ctx.addLine(
        `this.subTemplates['${subTemplate}'].call(this, Object.assign(Object.create(context), scope), Object.assign({}, extra, {parentNode: c${ctx.parentNode}}));`
      );
    } else {
      ...
      ctx.addLine(`result = []`);
      ctx.addLine(
        `this.subTemplates['${subTemplate}'].call(this, Object.assign(Object.create(context), scope), Object.assign({}, extra, {parentNode: result}));`
      );
